...
def plot_interval(ax,x,Y,std=True,n_sigma=3,min_max=True,label=None,linestyles=None,alpha=[None,0.75,0.5], **kwargs):
    """
    ...
    **kwargs will be passed to all subplots
    ...
    """
    Y_m = Y.mean(axis=0)
    line, = ax.plot(x, Y_m ,label=label,ls=linestyles,**kwargs)
    if std :
        Y_std = Y.std(axis=0)
